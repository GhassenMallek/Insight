...
export class GlobalService {
 ...
 uri: any ="http://localhost:3000" //"http://localhost:3000";
  // uri: any ="http://localhost:3000" // backend url
   PredectionUri :any = "http://localhost:5000"
   PredectionUrii :any = "http://127.0.0.1:8000"
  ...
  postFileToPyy(fileToUpload: File,) {
    const formData: FormData = new FormData();
    formData.append('file', fileToUpload, fileToUpload.name);
    return this._httpClient.post<any>(this.PredectionUrii+'/predict_disease',formData,
    );
  }
  postFileToPy(fileToUpload: File,) {
    const formData: FormData = new FormData();
    formData.append('file1', fileToUpload, fileToUpload.name);
    return this._httpClient.post<any>(this.PredectionUri,formData,
    );
  }
  isLoggedIn() {
